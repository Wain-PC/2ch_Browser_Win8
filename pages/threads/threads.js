...
function makeThreadsPostsList(json) {
    ...
    for (i = 0; i < threads.length; i++) { //work over each thread
        ...
        var numberOfLastPosts = 10; //number of posts (incl. OP-post) to show on the threads page. Now hardcoded to 4 as 1366*768 screens cannot handle more
...
function (item1, item2) {
    ...
    THREADS.threadsPostsList = groupedThreadsPostsList;
    THREADS.threadsPostsGroupes = groupedThreadsPostsList.groups;

    var listView = document.querySelector(".itemslist-threads").winControl;
    listView.itemDataSource = THREADS.threadsPostsList.dataSource;
    listView.groupDataSource = THREADS.threadsPostsGroupes.dataSource;
...
function navigateToThread(args) {
    var post = THREADS.threadsPostsList.getAt(args.detail.itemIndex);
    var opPost = THREADS.getOpPostByThreadNumber(post.threadNumber);
...
function getOpPostByThreadNumber(threadNumber) {
    for (var i = 0; i < THREADS.threadsPostsList.length; i++) {
        if (THREADS.threadsPostsList.getAt(i).postNum == threadNumber) {
            return THREADS.threadsPostsList.getAt(i);
        }
    }
}
